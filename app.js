const numberBox = document.getElementById("number-box");
const slider = document.getElementById("slider");
const progressBar = document.getElementById("progress-bar");
const playButton = document.getElementById("play-button");
// const pauseButton = document.getElementById("pause-button");
// console.log(playButton);
const queen = '<i class="fas fa-chess-queen" style="color:#000"></i>';
// console.log(numberBox.value);
let n,
  speed,
  tempSpeed,
  q,
  Board = 0;
let array = [0, 2, 1, 1, 3, 11, 5, 41, 93];
let pos = {};
//set slider value onSlide
speed = (100 - slider.value) * 10;
tempSpeed = speed;
slider.oninput = function () {
  progressBar.style.width = this.value + "%";
  speed = slider.value;
  speed = (100 - speed) * 10;
};
class Queen {
  constructor() {
    this.position = Object.assign({}, pos);
    this.unique_id = [];
  }
  nQueen = async () => {
    Board = 0;
    this.position[`${Board}`] = {};
    numberBox.disabled = true;
    await q.solveQueen(Board, 0, n);
    await q.clearColor(Board);
    numberBox.disabled = false;
  };
  //isValid function goes here.
  isValid = async (board, r, col, n) => {
    const table = document.getElementById(`table-${this.unique_id[board]}`);
    const currentRow = table.firstChild.childNodes[r];
    // console.log(currentRow);
    const currentCol = currentRow.getElementsByTagName("td")[col];
    currentCol.innerHTML = queen;
    await q.delay();
    //check queen in same col
    for (let i = r - 1; i >= 0; i--) {
      const row = table.firstChild.childNodes[i];
      const column = row.getElementsByTagName("td")[col];
      const value = column.innerHTML;
      console.log(`${board} and ${queen} and ${value}`);
      if (value == queen) {
        column.style.backgroundColor = "#FB5607";
        currentCol.innerHTML = "-";
        return false;
      }
      column.style.backgroundColor = "#ffca3a";
      await q.delay();
    }
    //check queen in upper left diagonal
    for (let i = r - 1, j = col - 1; i >= 0 && j >= 0; --i, --j) {
      const row = table.firstChild.childNodes[i];
      const column = row.getElementsByTagName("td")[j];
      const value = column.innerHTML;
      if (value == queen) {
        column.style.backgroundColor = "#FB5607";
        currentCol.innerHTML = "-";
        return false;
      }
      column.style.backgroundColor = "#ffca3a";
      await q.delay();
    }
    //check queen in upper right diagonal
    for (let i = r - 1, j = col + 1; i >= 0 && j < n; --i, ++j) {
      const row = table.firstChild.childNodes[i];
      const column = row.getElementsByTagName("td")[j];
      const value = column.innerHTML;
      if (value == queen) {
        column.style.backgroundColor = "#FB5607";
        currentCol.innerHTML = "-";
        return false;
      }
      column.style.backgroundColor = "#ffca3a";
      await q.delay();
    }
    return true;
  };
  //function to clear colors
  clearColor = async (board) => {
    for (let j = 0; j < n; j++) {
      const table = document.getElementById(`table-${this.unique_id[board]}`);
      const row = table.firstChild.childNodes[j];
      for (let k = 0; k < n; k++) {
        (j + k) & 1
          ? (row.getElementsByTagName("td")[k].style.backgroundColor =
              "#FF9F1C")
          : (row.getElementsByTagName("td")[k].style.backgroundColor =
              "#FCCD90");
      }
    }
  };
  //function to produce a delay
  delay = async () => {
    await new Promise((done) => setTimeout(() => done(), speed));
  };
  solveQueen = async (board, r, n) => {
    if (r == n) {
      Board++;
      let table = document.getElementById(`table-${this.unique_id[Board]}`);
      for (let k = 0; k < n; k++) {
        let row = table.firstChild.childNodes[k];
        row.getElementsByTagName("td")[this.position[board][k]].innerHTML =
          queen;
      }
      this.position[Board] = this.position[board];
      return;
    }

    for (let i = 0; i < n; i++) {
      await q.delay();
      await q.clearColor(board);
      if (await q.isValid(board, r, i, n)) {
        await q.delay();
        await q.clearColor(board);
        let table = document.getElementById(`table-${this.unique_id[board]}`);
        let row = table.firstChild.childNodes[r];
        row.getElementsByTagName("td")[i].innerHTML = queen;
        this.position[board][r] = i;
        if (await q.solveQueen(board, r + 1, n)) {
          await q.clearColor(board);
        }
        await q.delay();
        board = Board;
        table = document.getElementById(`table-${this.unique_id[board]}`);
        row = table.firstChild.childNodes[r];
        row.getElementsByTagName("td")[i].innerHTML = "-";
        delete this.position[`${board}`][`${r}`];
      }
    }
  };
}
//event on playButton
playButton.onclick = async () => {
  const chessBoard = document.getElementById("n-queen-board");
  const arrangement = document.getElementById("queen-arrangement");
  n = numberBox.value;
  q = new Queen();
  if (n > 8) {
    numberBox.value = "";
    alert("Queen value is too large");
    return;
  } else if (n < 1) {
    numberBox.value = "";
    alert("Queen value is too small");
    return;
  }
  //remove all previous execution context.
  while (chessBoard.hasChildNodes()) {
    chessBoard.removeChild(chessBoard.firstChild);
  }
  while (arrangement.hasChildNodes()) {
    arrangement.removeChild(arrangement.lastChild);
  }
  const para = document.createElement("p");
  para.innerHTML = `For ${n}x${n} board, ${
    array[n] - 1
  } arrangements are possible.`;
  arrangement.appendChild(para);
  //create boards in div.
  //   console.log(chessBoard.childElementCount);
  if (chessBoard.childElementCount === 0) {
    for (let i = 0; i < array[n]; i++) {
      q.unique_id.push(Math.random());
      let div = document.createElement("div");
      let table = document.createElement("table");
      let header = document.createElement("h4");
      header.innerHTML = `Board ${i + 1}`;
      table.setAttribute("id", `table-${q.unique_id[i]}`);
      header.setAttribute("id", `paragraph-${i}`);
      chessBoard.appendChild(div);
      div.appendChild(header);
      div.appendChild(table);
    }
  }
  for (let k = 0; k < array[n]; k++) {
    let table = document.getElementById(`table-${q.unique_id[k]}`);
    for (let i = 0; i < n; i++) {
      const row = table.insertRow(i);
      row.setAttribute("id", `Row${i}`);
      for (let j = 0; j < n; ++j) {
        const col = row.insertCell(j);
        // console.log(
        //   `value of i ${i} and value of j ${j} and value of (i+j)&1 ${
        //     (i + j) & 1
        //   }`
        // );
        (i + j) & 1
          ? (col.style.backgroundColor = "#FF9F1C")
          : (col.style.backgroundColor = "#FCCD90");
        col.innerHTML = "-";
        col.style.border = "0.3px solid #373f51";
      }
    }
    await q.clearColor(k);
  }

  await q.nQueen();
};
